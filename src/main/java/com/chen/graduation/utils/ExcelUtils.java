...
public class ExcelUtils {
    ...
    public static void exportLoginLog(HttpServletResponse response, List<LoginLogVO> listAjaxResultData) throws IOException {
        //设置响应参数
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setCharacterEncoding("utf-8");
        // 这里URLEncoder.encode可以防止中文乱码 当然和easyexcel没有关系
        String now = LocalDateTime.now().format(DateTimeFormatter.ofPattern("_yyyy-MM-dd_HH-mm-ss"));
        String fileName = URLEncoder.encode("LoginLog" + now, "UTF-8").replaceAll("\\+", "%20");
        response.setHeader("Content-disposition", "attachment;filename*=utf-8''" + fileName + ".xlsx");
        //写出文件
        EasyExcel.write(response.getOutputStream(), LoginLogVO.class).sheet("操作日志").doWrite(listAjaxResultData);
    }
}
